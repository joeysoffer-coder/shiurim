...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/api/admin/analytics' && req.method === 'GET') {
      ...
      return sendJson(res, 200, summarizeAnalytics(await loadAnalyticsEvents(days), days));
    }
...
const emptyLibraryConfig = () => ({ theme:'classic', showTodaysClasses:true, folders:[], rules:[], moves:[], hiddenFolders:[], hiddenPaths:[], pathTransforms:[], disabledBuiltInRules:[], builtInRuleEdits:{}, overrides:{} });
...
async function loadAnalyticsEvents(days) {
  const {url,key}=supabaseSettings();
  if(!url||!key)throw new Error('Supabase is not configured');
  const since=new Date(Date.now()-days*86400000).toISOString(),rows=[];
  for(let offset=0;offset<50000;offset+=1000){
    const endpoint=`${url}/rest/v1/analytics_events?created_at=gte.${encodeURIComponent(since)}&select=created_at,device_id,session_id,event_type,episode_id,episode_title,folder,value,platform,app_mode&order=created_at.asc&offset=${offset}&limit=1000`;
...
function summarizeAnalytics(events,days) {
  ...
  for(const event of events){
    ...
    if(event.episode_id){
      const item=episodes.get(event.episode_id)||{id:event.episode_id,title:event.episode_title||'Unknown episode',plays:0,completions:0,listenSeconds:0};
  ...
  return {
    days,eventCount:events.length,limited:events.length>=50000,
    totals:{uniqueDevices:unique.size,sessions:sessions.size,opens:actions.app_open||0,plays:actions.play_start||0,completions:actions.episode_complete||0,listenSeconds,searches:actions.search||0,downloads:actions.download||0,shares:(actions.share_episode||0)+(actions.share_app||0),contacts:actions.contact||0},
    ...
    topEpisodes:[...episodes.values()].sort((a,b)=>b.plays-a.plays||b.listenSeconds-a.listenSeconds).slice(0,20),
...
function sanitizeLibraryConfig(value) {
  ...
  return { theme, showTodaysClasses, folders, rules, moves, hiddenFolders, hiddenPaths, pathTransforms, disabledBuiltInRules, builtInRuleEdits, overrides };
}
