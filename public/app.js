...
function shareEpisode(id){
  ...
  shareLink({title:episode.title,text:`Listen to ${episode.title} on JS Torah`,url:url.toString()});
}
$('#shareAppBtn').addEventListener('click',()=>shareLink({
  title:'JS Torah',
  text:'Listen to Rabbi Joey Soffer Shiurim on JS Torah',
  url:location.origin
}));
...
episodeHTML=function(episode,index){
  ...
  const downloadButton=originalHTML.match(/<button class="download-btn"[\s\S]*?<\/button>/)?.[0]||'';
  const shareButton=(originalHTML.match(/<button class="share-episode-btn"[\s\S]*?<\/button>/)?.[0]||'').replace('Share episode','Share');
  const favoriteButton=`<button class="favorite-btn ${favorite?'is-favorite':''}" type="button" data-favorite="${esc(episode.id)}" aria-pressed="${favorite}">${favorite?'★ Favorited':'☆ Favorite'}</button>`;
