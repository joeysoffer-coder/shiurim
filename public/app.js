...
function fuzzyWordMatch(queryWord,textWord){if(queryWord===textWord)return 20;if(queryWord.length>=3&&(textWord.startsWith(queryWord)||queryWord.startsWith(textWord)))return 16;const allowance=queryWord.length<=3?0:queryWord.length<=4?1:queryWord.length<=7?2:3,distance=editDistance(queryWord,textWord);return distance<=allowance?12-distance:0}
...
audio.addEventListener('ended',()=>{
  ...
  save();
  render();
  const currentIndex=playbackQueue.indexOf(completedId),nextId=currentIndex>=0?playbackQueue[currentIndex+1]:null;
  if(nextId){setStatus('Playing next episode…');playEpisode(nextId,true)}
...
if('mediaSession'in navigator){
  ...
  setMediaAction('nexttrack',()=>{
    const index=state.current?playbackQueue.indexOf(state.current.id):-1,nextId=index>=0?playbackQueue[index+1]:null;
