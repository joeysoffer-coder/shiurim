...
function managedAssignment(episode){
  ...
  const originalFolder=folderInfo(title).name,move=(managedConfig.moves||[]).find(item=>item.source===originalFolder);
  if(!move?.parentPath?.length)return null;
...
function managedLibraryGroups(){
  ...
  (managedConfig.folders||[]).forEach(folder=>{const key=folder.name.toLocaleLowerCase();if(!candidates.has(key))candidates.set(key,{name:folder.name,forced:true,episodes:[]})});
  const folders=[...candidates.values()].filter(folder=>folder.forced||folder.episodes.length>1).sort((a,b)=>collator.compare(a.name,b.name));
  const groupedIds=new Set(folders.flatMap(folder=>folder.episodes.map(episode=>episode.id)));
...
audio.addEventListener('ended',()=>{
  ...
  save();
  render();
  const currentIndex=playbackQueue.indexOf(completedId),nextId=currentIndex>=0?(playbackQueue[currentIndex+1]||playbackQueue[0]):null;
  if(nextId){setStatus(currentIndex===playbackQueue.length-1?'Restarting from the first episode…':'Playing next episode…');playEpisode(nextId,true)}
  else setStatus('Episode complete.');
});
...
if('mediaSession'in navigator){
  ...
  setMediaAction('nexttrack',()=>{
    const index=state.current?playbackQueue.indexOf(state.current.id):-1,nextId=index>=0?(playbackQueue[index+1]||playbackQueue[0]):null;
...
render=function(){
  ...
  const results=searchEpisodes(state.episodes,query);
  $('#libraryTitle').textContent='Search results';
  $('#episodeCount').textContent=`${results.length} episode${results.length===1?'':'s'} across entire catalog`;
  list.innerHTML=results.length?results.map(episodeHTML).join(''):'<div class="empty">No exact or similar spellings were found.</div>';
};
render();
...
function applyNestedManagedFolderView(){
  ...
  for(const name of activeManagedPath){configuredNode=(configuredNode.children||[]).find(item=>item.name===name);if(!configuredNode)return}
  const currentPath=[activeFolder,...activeManagedPath];
  const assigned=state.episodes.map(episode=>({episode,assignment:managedAssignment(episode)})).filter(item=>item.assignment?.path?.slice(0,currentPath.length).every((part,index)=>part===currentPath[index]));
  const direct=sortEpisodes(assigned.filter(item=>item.assignment.path.length===currentPath.length).map(item=>item.episode));
  const children=new Map();
  assigned.filter(item=>item.assignment.path.length>currentPath.length).forEach(item=>{const name=item.assignment.path[currentPath.length];if(!children.has(name))children.set(name,[]);children.get(name).push(item.episode)});
