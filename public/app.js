...
let managedConfig={folders:[],rules:[],overrides:{}},activeManagedSubfolder=null;
...
function managedAssignment(episode){
  ...
  if(override?.folder)return{folder:override.folder,subfolder:override.subfolder||''};
  ...
  if(!rule)return null;
  let subfolder='';
  if(rule.strategy==='first_word')subfolder=capitalizeFolderLabel(title.trim().split(/\s+/)[0]||'Other');
  if(rule.strategy==='word_after')subfolder=wordsAroundMarker(title,rule.marker,'after');
  if(rule.strategy==='before_word')subfolder=wordsAroundMarker(title,rule.marker,'before');
  if(rule.strategy==='fixed')subfolder=rule.subfolder||'Other';
  return{folder:rule.folder,subfolder};
}
libraryGroups=function(){
  const candidates=new Map();
  state.episodes.forEach(episode=>{
    const managed=managedAssignment(episode),info=managed?{name:managed.folder,forced:true}:folderInfo(episode.title),key=info.name.toLocaleLowerCase();
    if(!candidates.has(key))candidates.set(key,{name:info.name,forced:info.forced,episodes:[]});
