...
function dafFolderName(title=''){
  ...
  return beforeDaf.replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu,'').trim()||'Daf Yomi';
}
...
render=function(){
  ...
  const folders=[...subfolders.entries()].sort((a,b)=>collator.compare(a[0],b[0]));
...
function rashiFolderName(title=''){
  return title.trim().split(/\s+/)[0]?.replace(/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu,'')||'Other';
}
