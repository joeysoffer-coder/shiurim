...
function applyNestedManagedFolderView(){
  if($('#searchInput').value.trim()||!activeFolder)return;
  const configured=managedConfig.folders?.some(item=>item.name===activeFolder);
  ...
  const classes=direct.length?`${childFolders.length?'<h3 class="section-label">Classes in this folder</h3>':''}${direct.map(episodeHTML).join('')}`:'';
