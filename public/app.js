...
episodeHTML=function(episode,index){
  ...
  return episodeHTMLWithSharing(episode,index).replace('</div></article>',`<button class="favorite-btn ${favorite?'is-favorite':''}" type="button" data-favorite="${esc(episode.id)}" aria-pressed="${favorite}">${favorite?'★ Favorited':'☆ Favorite'}</button></div></article>`);
};
