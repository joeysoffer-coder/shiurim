...
function searchableFolderPaths(){
  ...
  state.episodes.forEach(episode=>{const assignment=managedAssignment(episode);if(assignment?.path&&!managedPathHidden(assignment.path))for(let length=1;length<=assignment.path.length;length++)paths.push(assignment.path.slice(0,length))});
...
let managedConfig={theme:'classic',folders:[],rules:[],moves:[],hiddenFolders:[],hiddenPaths:[],pathTransforms:[],overrides:{}},activeManagedPath=[],activeManagedSubfolder=null;
...
async function loadManagedConfig(){
  try{
    ...
    if(response.ok){managedConfig=await response.json();applyAppTheme(managedConfig.theme);filingRuleConfig={disabledBuiltInRules:managedConfig.disabledBuiltInRules||[],builtInRuleEdits:managedConfig.builtInRuleEdits||{}};folderInfoCache.clear();managedAssignmentCache.clear();managedGroupsCache={episodes:null,config:null,value:null};render()}
  }catch{}
}
...
const helpTour=$('#helpTour'),helpTrack=$('#helpTrack'),helpPages=[...document.querySelectorAll('.help-page')],helpDots=$('#helpDots');
let helpPage=0,helpTouchStart=0,helpPreviousFocus=null;
helpDots.innerHTML=helpPages.map((_,index)=>`<button type="button" data-help-page="${index}" aria-label="Go to help page ${index+1}"></button>`).join('');
function showHelpPage(index){
  helpPage=Math.max(0,Math.min(helpPages.length-1,index));helpTrack.style.transform=`translateX(-${helpPage*100}%)`;
  [...helpDots.children].forEach((dot,dotIndex)=>dot.classList.toggle('active',dotIndex===helpPage));
  $('#helpBack').disabled=helpPage===0;$('#helpNext').textContent=helpPage===helpPages.length-1?'Done':'Next';
  helpPages[helpPage].scrollTop=0;
}
function openHelp(){
  helpPreviousFocus=document.activeElement;helpTour.hidden=false;document.body.style.overflow='hidden';showHelpPage(0);trackAnalytics('help_open');$('#helpClose').focus();
}
function closeHelp(completed=false){
  if(helpTour.hidden)return;helpTour.hidden=true;document.body.style.overflow='';if(completed)trackAnalytics('help_complete');helpPreviousFocus?.focus();
}
$('#helpBtn').addEventListener('click',openHelp);
$('#helpClose').addEventListener('click',()=>closeHelp(false));
$('#helpExit').addEventListener('click',()=>closeHelp(false));
$('#helpBack').addEventListener('click',()=>showHelpPage(helpPage-1));
$('#helpNext').addEventListener('click',()=>helpPage===helpPages.length-1?closeHelp(true):showHelpPage(helpPage+1));
...
helpTour.addEventListener('click',event=>{if(event.target===helpTour)closeHelp(false)});
$('#helpViewport').addEventListener('touchstart',event=>{helpTouchStart=event.changedTouches[0].clientX},{passive:true});
$('#helpViewport').addEventListener('touchend',event=>{const distance=event.changedTouches[0].clientX-helpTouchStart;if(Math.abs(distance)>45)showHelpPage(helpPage+(distance<0?1:-1))},{passive:true});
document.addEventListener('keydown',event=>{if(helpTour.hidden)return;if(event.key==='Escape')closeHelp(false);else if(event.key==='ArrowRight')showHelpPage(helpPage+1);else if(event.key==='ArrowLeft')showHelpPage(helpPage-1)});
