...
let managedConfig={theme:'classic',showTodaysClasses:true,folders:[],rules:[],moves:[],hiddenFolders:[],hiddenPaths:[],pathTransforms:[],overrides:{}},activeManagedPath=[],activeManagedSubfolder=null;
...
async function loadManagedConfig(){
  ...
  try{
    ...
    if(response.ok){managedConfig=await response.json();showTodaysClasses=managedConfig.showTodaysClasses!==false;applyAppTheme(managedConfig.theme);filingRuleConfig={disabledBuiltInRules:managedConfig.disabledBuiltInRules||[],builtInRuleEdits:managedConfig.builtInRuleEdits||{}};folderInfoCache.clear();managedAssignmentCache.clear();managedGroupsCache={episodes:null,config:null,value:null};render()}
