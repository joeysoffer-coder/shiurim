...
function automaticAssignmentPath(episode){
  ...
  if(override?.path?.length)return override.path;
  ...
  if(move?.parentPath?.length)return[...move.parentPath,move.name||original];
  ...
  return rule?.path?.length?rule.path:null;
}
...
async function load(){
  ...
  refreshSelects();renderCurrentFolders();renderUnfiledClasses();
}
async function save(){
  ...
  try{
    ...
    if(revision===changeRevision){config=data;normalizeConfig();dirty=false;$('saveMessage').textContent='Saved. The listener app is updated.';$('saveMessage').className='ok';renderFolders();renderRules();renderCurrentFolders();renderUnfiledClasses()}
...
$('addFolder').onclick=()=>{const name=cap($('newFolder').value);if(!name||config.folders.some(f=>f.name.toLowerCase()===name.toLowerCase()))return;config.folders.push({name,children:[]});$('newFolder').value='';changed();renderFolders();renderCurrentFolders()};
$('folderList').onclick=event=>{
  ...
  changed();renderFolders();renderRules();renderCurrentFolders();
};
$('currentFolderList').onclick=event=>{
  ...
  if(source){
    ...
    if(event.target.dataset.customFolder==='true'){
      ...
      changed();renderFolders();renderCurrentFolders();return;
    }
...
$('saveAssignment').onclick=()=>{const path=readPath($('assignmentFolder').value);if(!selected||!path.length)return alert('Choose a destination.');config.overrides[String(selected.id)]={path};changed();renderUnfiledClasses()};
$('removeAssignment').onclick=()=>{if(!selected)return;delete config.overrides[String(selected.id)];$('assignmentFolder').value='';changed();renderUnfiledClasses()};
