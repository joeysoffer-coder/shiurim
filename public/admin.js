...
function listeningFolderPaths(){
  const paths=allPaths().map(path=>({path,episode:null})),counts=new Map(),forced=new Set(BUILTIN_RULES.map(rule=>rule[1]));
  ...
  episodes.forEach(episode=>{
    ...
    if(config.hiddenFolders.includes(path[0])||config.hiddenFolders.includes(currentFolderName(episode.title)))return;
...
function renderListeningFolderTree(){
  ...
  const html=nodes=>[...nodes.values()].sort((a,b)=>a.name.localeCompare(b.name,undefined,{numeric:true,sensitivity:'base'})).map(node=>`<article class="folder-card tree-level"><div class="folder-head"><span class="folder-name">${esc(node.name)}</span><span class="sub">${node.ids.size} episode${node.ids.size===1?'':'s'}</span></div><div class="nested-tree">${html(node.children)}</div></article>`).join('');
...
async function load(){
  ...
  refreshSelects();renderCurrentFolders();renderListeningFolderTree();renderUnfiledClasses();
}
async function save(){
...
$('saveAssignment').onclick=()=>{const path=readPath($('assignmentFolder').value);if(!selected||!path.length)return alert('Choose a destination.');config.overrides[String(selected.id)]={path};changed();renderListeningFolderTree();renderUnfiledClasses()};
