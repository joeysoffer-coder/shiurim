...
let token=sessionStorage.getItem('rjsAdminToken')||'',config={theme:'classic',folders:[],rules:[],moves:[],hiddenFolders:[],hiddenPaths:[],pathTransforms:[],disabledBuiltInRules:[],builtInRuleEdits:{},overrides:{}},episodes=[],selected=null,dirty=false;
...
function renderAnalytics(data){
  const totals=data.totals||{},cards=[
    ...
    ['Searches',analyticsNumber(totals.searches)],['Downloads',analyticsNumber(totals.downloads)],['Shares',analyticsNumber(totals.shares)],['Contact clicks',analyticsNumber(totals.contacts)]
  ];
  ...
  const daily=(data.daily||[]).slice(-30),maxDaily=Math.max(1,...daily.map(day=>day.plays));
  $('analyticsDaily').innerHTML=daily.length?`<div class="daily-chart">${daily.map(day=>`<div class="daily-column" title="${esc(day.date)}: ${day.plays} plays"><i style="height:${Math.max(4,day.plays/maxDaily*100)}%"></i><span>${esc(day.date.slice(5))}</span></div>`).join('')}</div><p class="sub">Bars show episode plays. Hover or press a bar for the date and count.</p>`:'<p class="sub">No daily activity yet.</p>';
  ...
  $('analyticsEpisodes').innerHTML=top.length?`<div class="analytics-table-wrap"><table class="analytics-table"><thead><tr><th>Episode</th><th>Plays</th><th>Completed</th><th>Listening</th></tr></thead><tbody>${top.map(item=>`<tr><td>${esc(item.title)}</td><td>${analyticsNumber(item.plays)}</td><td>${analyticsNumber(item.completions)}</td><td>${analyticsDuration(item.listenSeconds)}</td></tr>`).join('')}</tbody></table></div>`:'<p class="sub">Episode activity will appear after listeners use the updated app.</p>';
  $('analyticsFolders').innerHTML=analyticsRows(data.topFolders||[],'opens','Folder activity will appear here.');
  const actionLabels={play_start:'Episode plays',episode_complete:'Completed episodes',listen_time:'Listening updates',folder_open:'Folder opens',search:'Searches',download:'Downloads',download_remove:'Downloads removed',favorite:'Favorites added',favorite_remove:'Favorites removed',share_episode:'Episodes shared',share_app:'App shares',contact:'Contact clicks',app_open:'App opens'};
  ...
  $('analyticsMessage').textContent=data.limited?'Showing the first 50,000 events in this period. Choose a shorter period for exact totals.':`Updated just now · Last ${data.days} days`;
  $('analyticsMessage').className=data.limited?'message':'message ok';
}
async function loadAnalytics(){
  if(analyticsLoading)return;analyticsLoading=true;$('refreshAnalytics').disabled=true;$('analyticsMessage').textContent='Loading analytics…';$('analyticsMessage').className='message';
  try{const response=await fetch(`/api/admin/analytics?days=${encodeURIComponent($('analyticsDays').value)}`,{headers:auth()}),data=await response.json().catch(()=>({error:'Analytics could not be loaded.'}));if(!response.ok)throw Error(data.error||'Analytics could not be loaded.');renderAnalytics(data)}
...
function renderPanel(panel){
  ...
  else if(panel==='analytics')loadAnalytics();
}
