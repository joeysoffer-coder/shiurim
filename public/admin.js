...
function currentFolderName(title=''){
  const clean=String(title).trim().replace(/\s+/g,' '),matched=builtInMatch(clean);
  if(matched)return cap(matched.folder);
...
async function save(){
  ...
  try{
    ...
    if(revision===changeRevision){config=data;normalizeConfig();dirty=false;$('saveMessage').textContent='Saved. The listener app is updated.';$('saveMessage').className='ok';renderFolders();renderRules();renderBuiltInRules();renderCurrentFolders();renderListeningFolderTree();renderUnfiledClasses()}
...
$('builtInRuleList').onclick=event=>{
  ...
  changed();renderBuiltInRules();renderCurrentFolders();renderListeningFolderTree();renderUnfiledClasses();
};
