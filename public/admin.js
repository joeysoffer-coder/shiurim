...
let token=sessionStorage.getItem('rjsAdminToken')||'',config={folders:[],rules:[],moves:[],overrides:{}},episodes=[],selected=null,dirty=false;
...
function renderCurrentFolders(){
  ...
  $('currentFolderList').innerHTML=folders.length?folders.map(([name,items])=>{
    const move=config.moves.find(item=>item.source===name),selected=move?.parentPath||[],location=move?`${move.parentPath.join(' › ')} › ${move.name||name}`:'Home screen';
    return `<article class="current-folder"><div><strong>${esc(name)}</strong><small>${items.length} episodes · Currently: ${esc(location)}</small></div>
      <div class="move-controls"><select data-move-parent="${esc(name)}"><option value="">Move inside…</option>${destinationOptions(selected)}</select><input data-move-name="${esc(name)}" value="${esc(move?.name||name)}" aria-label="Folder name after move"><button data-move-folder="${esc(name)}">Move</button>${move?`<button class="danger" data-unmove-folder="${esc(name)}">Undo move</button>`:''}</div></article>`;
...
async function save(){
  ...
  config=data;normalizeConfig();dirty=false;$('saveMessage').textContent='Saved. The listener app is updated.';$('saveMessage').className='ok';renderFolders();renderRules();renderCurrentFolders();
}
...
$('currentFolderList').onclick=event=>{
  const source=event.target.dataset.moveFolder,undo=event.target.dataset.unmoveFolder;
  if(source){const select=document.querySelector(`[data-move-parent="${CSS.escape(source)}"]`),nameInput=document.querySelector(`[data-move-name="${CSS.escape(source)}"]`),parentPath=readPath(select?.value);if(!parentPath.length)return alert('Choose the master folder where this folder should go.');config.moves=config.moves.filter(item=>item.source!==source);config.moves.push({source,name:cap(nameInput?.value)||source,parentPath});changed();renderCurrentFolders()}
  else if(undo){config.moves=config.moves.filter(item=>item.source!==undo);changed();renderCurrentFolders()}
};
